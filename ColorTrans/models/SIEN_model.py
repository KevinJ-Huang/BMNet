...
####################################################################
    ...
    def feed_data(self, data, need_GT=True):
        ...
        self.mask = MASK_IMG.to(self.device)
        # self.varright_L = LQright_IMG.to(self.device)
        if need_GT:
            self.real_H = GT_IMG.to(self.device)
            self.real_H = self.real_H/(torch.mean(self.real_H,1).unsqueeze(1)+1e-8)
    ...
    def optimize_parameters(self, step):
        ...
        out = self.netG(self.var_L,self.mask)
        gt = self.real_H

        # histgt = histcal(self.real_H.detach())
        # histout = histcal(out)

        l_total = self.cri_pix(out, gt)
                  # +5*self.cri_pix(out*self.mask,gt*self.mask)
                  # +0.5*self.cri_pix(maskest,self.mask.detach())
                  #
        ...
        self.log_dict['psnr_rev'] = psnr.item()
